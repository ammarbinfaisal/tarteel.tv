...
function assertClip(clip) {
  ...
  if (typeof clip.reciter !== "string" || !clip.reciter) throw new Error(`Clip.reciter invalid for id=${clip.id}`);
...
function buildIndex(clips) {
  ...
  const byReciter = Object.create(null);
  ...
  for (const clip of clips) {
    ...
    const translation = clip.translation ?? "saheeh-international";

    byId[clip.id] = { ...clip, riwayah, translation };
    (bySurah[String(clip.surah)] ??= []).push(clip.id);
    (byReciter[clip.reciter] ??= []).push(clip.id);
